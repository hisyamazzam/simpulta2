        ...
        return $form
            ->schema([
                ...
                Forms\Components\FileUpload::make('abstrakindonesia')
                    ->directory(fn ($get) => 'skripsi/' . $get('nim') . '_' . $get('nama')) // Ambil nilai langsung dari input
                    ->required(),
                Forms\Components\FileUpload::make('abstrakinggris')
                    ->directory(fn ($get) => 'skripsi/' . $get('nim') . '_' . $get('nama'))
                    ->required(),
                Forms\Components\FileUpload::make('lembarpengesahan')
                    ->directory(fn ($get) => 'skripsi/' . $get('nim') . '_' . $get('nama'))
                    ->required(),
                Forms\Components\FileUpload::make('skripsi')
                    ->directory(fn ($get) => 'skripsi/' . $get('nim') . '_' . $get('nama'))
                    ->required(),
            ]);
    }
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                ...
                Tables\Columns\IconColumn::make('skripsi')
                    ->label('Skripsi')
                    ->boolean()
                    ->trueIcon('heroicon-o-document-text')
                    ->falseIcon('heroicon-o-x-circle')
                    ->tooltip(fn ($record) => $record->skripsi ? 'Download Skripsi' : 'Tidak Ada File')
                    ->url(fn ($record) => $record->skripsi ? asset('storage/' . $record->skripsi) : null),
            ])
            ->filters([
